...
BEGIN
    ...
    SELECT @Instance + ' at ' + CONVERT(NVARCHAR(17), GETDATE(), 113) + ' Raporun alindiği sunucu ve tarih';
    INSERT INTO @SQL_Status_Report
    (
        Information
    )
    ...
    SELECT 'Disk durumu ' + @Instance + ' (En az alanli birim en üstte)';
    ...
    SELECT drive + ' sürücüsünde  ' + CAST(MbFree / 1000 AS NVARCHAR(20)) + ' GB bos alan var'
    FROM @drives
    ...
    SELECT 'Son ' + CAST(@DaysBack AS NVARCHAR(12)) + ' günde eklenen yeni kullanicilar';
    INSERT INTO @SQL_Status_Report
    (
        Information
    )
    ...
    SELECT @Instance + ' Sunucusunun Son ' + CAST(@Log_Days_Agent AS NVARCHAR(12)) + ' güne ait agent loglari';
